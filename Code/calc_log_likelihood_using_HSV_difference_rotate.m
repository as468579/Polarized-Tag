...
for k = 1:N
    ...
    if X(5,k) > -359 && X(5,k) < 359 && X(7,k) > 0 
        ...
        % Since we arrange our polarizer by column
        % ex : [ 1 ] + [ 2 ] + [ 0 ] + [ 1 ] + [ 2 ]+ [ 0 ] + [ 1 ] + [ 2 ] 
        %          0         0          1          1          1        2          2          2
        ...
        if I && J && M_resolution && N_resolution
            ...
            for i = 2:size(hueVector,2)
                for j = i-1:-1:1
                    ...
                    hueDiff(sum((i-2):-1:1)+j) = HueDistance(hueVector(i), hueVector(j));
                end
            end
            ...
            % 此處會順便過濾多個particle在同一個quarter的情況
            % 但3.6有點太鬆，可以調高試試看
...
%                 hold on
%                 for idx = 1:8
%                     plot(comparingCoord(2, idx), comparingCoord(1, idx), 'k*');
%                 end
